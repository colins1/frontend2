export default class Logic {
    ...
    async modalSubmit(e) {
      ...
      if (name === 'edit') {
        const request = {'title': e.target[0].value, 'description': e.target[1].value, 'id': id};
        (await (this.sendXHR('POST', JSON.stringify(request), 'editTicket')));
      } else if (name === 'del') {
        const request = id;
        (this.sendXHR('DELETE', request));
      } else if (name === 'createTicket') {
        const request = {'title': e.target[0].value, 'description': e.target[1].value};
        const result = JSON.parse(await (this.sendXHR('POST', JSON.stringify(request), 'createTicket')));
        this.gui.list.innerHTML += this.gui.rowTemplate(
          result.id,
          result.status,
          result.name,
